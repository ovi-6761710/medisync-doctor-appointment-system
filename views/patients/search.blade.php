...
@section ("main")
                        ...
                        <div class="card-body">
                            ...
                            <div class="filter-widget">
                                ...
                                @foreach ($specialities as $speciality)
                                    <div>
                                        <label class="custom_check">
                                            <input type="checkbox" name="speciality" value="{{ $speciality->name ?? '' }}"
                                                {{ in_array($speciality->name ?? '', $searched_specialities) ? "checked" : "" }} />
    ...
    <input type="hidden" id="input-location" value="{{ $location }}" />
    <input type="hidden" id="input-search" value="{{ $search }}" />
    ...
    <script>
        ...
        function doSearch() {
            ...
            for (let a = 0; a < gender.length; a++) {
                genderArr.push(gender[a].value || "");
            }
            url += "&gender=" + JSON.stringify(genderArr);

            const speciality = document.querySelectorAll("input[name='speciality']:checked");
            const specialitiesArr = [];
            for (let a = 0; a < speciality.length; a++) {
                specialitiesArr.push(speciality[a].value || "");
            }
            url += "&specialities=" + JSON.stringify(specialitiesArr);

            window.location.href = baseUrl + "/search?" + url;
        }
    </script>
